...
chassis_log *chassis_log_new(void) {
	...
	log->log_file_fd = -1;
	log->log_ts_str = g_string_sized_new(sizeof("2004-01-01T00:00:00.000000Z"));
...
const char *chassis_log_get_level_name(GLogLevelFlags lvl) {
	gint i;

	for (i = 0; log_lvl_map[i].name; i++) {
		if (lvl == log_lvl_map[i].lvl) {
...
static int chassis_log_update_timestamp(chassis_log *log) {
	...
	} else if (log->log_ts_resolution == CHASSIS_RESOLUTION_US) {
...
void chassis_set_logtimestamp_resolution(chassis_log *log, int res) {
	...
	if (res == CHASSIS_RESOLUTION_SEC || res == CHASSIS_RESOLUTION_MS || res == CHASSIS_RESOLUTION_US)
...
int chassis_log_set_event_log(chassis_log *log, const char *UNUSED_PARAM(app_name)) {
