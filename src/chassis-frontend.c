...
#ifdef GLIB_MEMORY_USAGE
	g_mem_set_vtable(glib_mem_profiler_table);
#endif

	if (!GLIB_CHECK_VERSION(2, 34, 0)) {
		g_critical("the glib header are too old, need at least 2.32.0, got: %d.%d.%d",
...
int chassis_frontend_init_base_options(GOptionContext *option_ctx,
	...
	/*@fixme 不懂，为什么不能用chassis_options_free_g_option_entries()释放entries？ */
