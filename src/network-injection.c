...
guint _dump_injected_queries(const char *name, network_injection_queue *q) {
	g_debug("[%s] queries length: %d", name, q->length);
	if (q->length > 0) {
		guint i;
		for (i = 0; i < q->length; i++) {
			GString *s;
			s = g_queue_peek_nth(q, i);
			g_debug("[%s] query: %d %s", name, i, s->str);
		}
	}
	return q->length;
}
...
GString *injection_queue_pop_head_injected_query(network_injection_queue *q) {
	injection *inj = NULL;
	GString *query = NULL;
	inj = g_queue_pop_head(q);
	if (inj != NULL ) {
		query = inj->query;
		inj->query = NULL;
		injection_free(inj);
