...
if ((num<3));then
	...
	ip_add="127.0.0.1:7766,127.0.0.1:7767,127.0.0.1:7768"
...
##### 删除第一个监听ip地址  ##############
listeners=$($mysql_admin "$show_listen"|grep rw |awk '{print $2}'|sed 's/,/\n/g'|grep -E -v '^$'|sed ':a; N;s/\n/,/;t a')
middle_addr=${listeners/,*}
middle_addr=${middle_addr#*,}
middle_addr=${middle_addr//,*}

$mysql_admin "dellistenaddr --backend=$middle_addr --bktype=rw" >/dev/null 2>&1
if [ $? -ne 0 ];then
	echo "listen addr:$middle_addr deleted error"
	exit 1
fi
...
##### 将删除的端口添加进来 ##############
$mysql_admin "addlistenaddr --backend=$middle_addr --bktype=rw" >/dev/null 2>&1
