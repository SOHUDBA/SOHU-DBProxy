...
START_TEST(test_log_timestamp) {
	...
	chassis_set_logtimestamp_resolution(l, CHASSIS_RESOLUTION_US);
	g_critical("this message has a usecond-resolution timestamp");
	g_assert_cmpint(CHASSIS_RESOLUTION_US, ==, chassis_get_logtimestamp_resolution(l));
...
/**
 * @test Test log set & get level
 */
START_TEST(test_log_level) {
	chassis_log *l;

	l = chassis_log_new();
	g_assert_cmpint(chassis_log_set_level(l, "error"), ==, 0);
	g_assert_cmpint(l->min_lvl, ==, G_LOG_LEVEL_ERROR);
	g_assert_cmpstr(chassis_log_get_level_name(l->min_lvl), ==, "error");

	g_assert_cmpint(chassis_log_set_level(l, "critical"), ==, 0);
	g_assert_cmpint(l->min_lvl, ==, G_LOG_LEVEL_CRITICAL);
	g_assert_cmpstr(chassis_log_get_level_name(l->min_lvl), ==, "critical");

	g_assert_cmpint(chassis_log_set_level(l, "warning"), ==, 0);
	g_assert_cmpint(l->min_lvl, ==, G_LOG_LEVEL_WARNING);
	g_assert_cmpstr(chassis_log_get_level_name(l->min_lvl), ==, "warning");

	g_assert_cmpint(chassis_log_set_level(l, "message"), ==, 0);
	g_assert_cmpint(l->min_lvl, ==, G_LOG_LEVEL_MESSAGE);
	g_assert_cmpstr(chassis_log_get_level_name(l->min_lvl), ==, "message");

	g_assert_cmpint(chassis_log_set_level(l, "info"), ==, 0);
	g_assert_cmpint(l->min_lvl, ==, G_LOG_LEVEL_INFO);
	g_assert_cmpstr(chassis_log_get_level_name(l->min_lvl), ==, "info");

	g_assert_cmpint(chassis_log_set_level(l, "debug"), ==, 0);
	g_assert_cmpint(l->min_lvl, ==, G_LOG_LEVEL_DEBUG);
	g_assert_cmpstr(chassis_log_get_level_name(l->min_lvl), ==, "debug");

	g_assert_cmpint(chassis_log_set_level(l, "aaa"), ==, -1);

	g_assert(chassis_log_get_level_name(-1) == NULL);
	g_assert(chassis_log_get_level_name(1234) == NULL);
...
int main(int argc, char **argv) {
	...
	g_test_add_func("/core/log_level", test_log_level);
